...
const addProduct = async (req, res) => {
    try {
        ...
        if(!title || !price || !category || !quantity) return res.status(400).json({error: "required title, price, category and quantity"});
        ...
        if(isProductInserted.rowCount > 0) {
            return res.status(409).json({error: "product with this title has already been inserted."});
        }
...
const showAllCategories = (req, res) => {
    try {
        ...
        db.query(q, (err, data) => {
            ...
            if(data.rowCount === 0) return res.status(404).json({error: "no product category found"});
...
const showProductsByCategoryId = (req, res) => {
    try {
        ...
        db.query(q, [catId], (err, data) => {
            if(err) return res.status(500).json({error: "internal server error"});
            if(data.rowCount === 0) return res.status(404).json({error: "no product found"});
...
router.put("/updateProductQuantity", jwtAuthMiddleWare, updateProductQuantity);
