...
const placeOrder = async (req, res) => {
    try {
        ...
        // if cart is empty
        if(cartItems.rowCount === 0) return res.status(404).json({error: "your cart is empty"});
        ...
        for(let i=0; i<cartItems.rowCount; i++) {
            ...
            if(quantity > availableQuantity) 
                return res.status(409).json({error: "your cart has more quantity than available quantities", availableQuantity});
        }
...
const viewOrders = async (req, res) => {
    try {
        ...
        if(orderIdsData.rowCount === 0) return res.status(400).js({message: "you have not placed any order."});
...
const viewOrderById = async (req, res) => {
    try {
        ...
        if(userDetail.rowCount === 0) return res.status(404).json({error: "no order with this id exist"});
        if(userName != userDetail.rows[0].user_name) return res.status(401).json({error: "this order does not belong to you"});
        
        // orderId belongs to current user
        const q = "SELECT p.product_id, title, description, price, os.order_id, os.order_timing, od.quantity FROM order_summary AS os INNER JOIN order_details AS od ON os.order_id = od.order_id INNER JOIN products AS p ON od.product_id = p.product_id WHERE os.user_name = $1 AND os.order_id = $2";
